{
  flake.modules.services.airsonic =
    { config, lib, pkgs, ... }:
    let
      inherit (config.services) airsonic caddy nginx;
    ...
    in {
      services = {
        ...
        caddy.virtualHosts."http://airsonic.fluorine.lan".extraConfig = lib.mkIf caddy.enable ''
          reverse_proxy http://127.0.0.1:${port}
        '';

        nginx.virtualHosts."airsonic.flourine.lan" = lib.mkIf nginx.enable {
          enableACME = false;
          forceSSL = false;
