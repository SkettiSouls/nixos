{
  flake.modules.services.gonic =
    ...
    in {
      options.services.gonic.port = lib.mkOption {
        type = lib.types.int;
        default = 4747;
      };
      ...
      config.services = {
        ...
        caddy.virtualHosts."http://gonic.fluorine.lan".extraConfig = lib.mkIf caddy.enable ''
          reverse_proxy http://127.0.0.1:${port}
        '';

        nginx.virtualHosts."gonic.flourine.lan" = lib.mkIf nginx.enable {
          enableACME = false;
          forceSSL = false;
